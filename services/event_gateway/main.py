...
**격리되는 것은 프로세스이지 코드가 아니다.** 이 모듈은 오케스트레이터의
DB 배선(`orchestrator.db`)과 ORM 모델(`pump.py`의 `orchestrator.models`)을
그대로 import한다. 아웃박스 테이블의 스키마는 오케스트레이터가 소유하므로,
그 정의를 여기서 베껴 쓰면 두 벌이 조용히 어긋난다 — 한쪽만 컬럼을 늘리면
런타임에야 드러난다. 공유하는 것은 **읽기 전용 스키마 지식**뿐이고, 공유하지
않는 것이 본질이다: 자기 자신의 DB 연결, 자기 자신의 FastAPI 앱, 자기 자신의
생명주기. 여기서 `Base.metadata.create_all`을 부르지 않는 것도 같은 선 긋기다
(스키마를 만드는 쪽은 오케스트레이터 하나여야 한다).

즉 오케스트레이터가 죽어도 이 프로세스는 계속 돌고, 이 프로세스가 죽어도
워크플로는 계속 돈다. 그 독립성은 import 그래프가 아니라 **프로세스 경계와
아웃박스 테이블**이 만든다.
